...
public static class DiscoverableHelper
{
    ...
    private static readonly WeakReference<List<Type>> _allTypesRef = new WeakReference<List<Type>>(null);
    ...
    public static IReadOnlyList<Type> FindManyByName<T>(string name)
        => Filter<T>()
            ...
            .Select(x => x.Type)
            .ToList();
    public static IReadOnlyList<Type> FindManyByName<T>(Enum name) => FindManyByName<T>(name.ToEnumString());
