...
namespace Snap.Paths
{
	public class GraphPathfinder
	{
		...
		private struct Edge
		{
			public int To;
			public float Cost;
			public Edge(int to, float cost) { To = to; Cost = cost; }
		}

		// --- Core graph storage ---
		private readonly List<List<Edge>> _adj = new List<List<Edge>>();
		private readonly List<int> _indexToId = new List<int>();
		private readonly Dictionary<int, int> _idToIndex = new Dictionary<int, int>();
		private readonly List<Vect2> _positions = new List<Vect2>();
		private readonly List<bool> _disabled = new List<bool>();
		...
		private readonly GraphPriorityQueue<int, float> _openSet
			= new GraphPriorityQueue<int, float>();
		...
		public void Connect(int fromId, int toId, float cost = 1f, bool bidirectional = true)
		{
		...
		public bool IsConnected(int fromId, int toId)
		{
			if (!_idToIndex.TryGetValue(fromId, out var from) ||
				!_idToIndex.TryGetValue(toId, out var to))
				return false;
		...
		public bool IsDisabled(int id) => _idToIndex.TryGetValue(id, out var i) && _disabled[i];
